...
- Our MOSSLET app uses `<.phx_flash_group>` and often the `phx` prefix to components to distinguish our components from the external `PetalComponents` library
- We **never** want to use `PetalComponents` as we are phasing them out and want to replace them with our `design_system.ex` and `core_components.ex`
...
- **Always** defer to our our design system in our `DESIGN_SYSTEM.md` and utilizing our `DESIGN_SYSTEM.ex` components to ensure a consistent style
- Focus on **visual hiearachy** and **accessibility**
