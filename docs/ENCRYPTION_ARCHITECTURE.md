...
### Encrypted List Fields - Context-Specific Patterns

**Two distinct patterns based on data ownership:**

#### Pattern A: User Personal Data (UserTimelinePreference)

```elixir
# USER-SPECIFIC DATA: Encrypt with user_key, then use StringList
field :muted_users, Mosslet.Encrypted.StringList      # Each user_id encrypted with user_key
field :muted_keywords, Mosslet.Encrypted.StringList   # Each keyword encrypted with user_key
```

**Implementation (User Personal Data):**

```elixir
# Personal preferences - encrypt each item with user_key
defp encrypt_muted_users(changeset, opts) do
  if changeset.valid? && opts[:user] && opts[:key] do
    ...
    if muted_users && length(muted_users) > 0 do
      ...
      changeset
      |> put_change(:muted_users, encrypted_user_ids)  # StringList encrypts the list
      |> put_change(:muted_users_hash, create_hash(muted_users))
    end
  end
end
```

#### Pattern B: Shareable Content Data (Posts, Bookmarks, etc.)

```elixir
# SHAREABLE DATA: Encrypt with context key (post_key, group_key, etc.)
field :tags, Mosslet.Encrypted.StringList             # Each tag encrypted with post_key
field :mentions, Mosslet.Encrypted.StringList         # Each mention encrypted with post_key
```

**Implementation (Shareable Content Data):**

```elixir
# Post-related data - encrypt each item with POST_KEY (shareable context)
defp encrypt_post_tags(changeset, opts) do
  if changeset.valid? && opts[:post_key] do
    ...
    if tags && length(tags) > 0 do
      ...
      changeset
      |> put_change(:tags, encrypted_tags)  # StringList encrypts the list
      |> put_change(:tags_hash, create_hash(tags))
    end
  end
end
```
...
- ✅ **User Personal Data** → `user_key` encryption (UserTimelinePreference, personal settings)
- ✅ **Shareable Content** → `context_key` encryption (post_key, group_key, etc.)
- ✅ **Access Control** → Context keys shared via user_post.key, user_group.key patterns

**Why This Matters:**

- 🔒 **Personal data** (muted users/keywords) → Only the user can decrypt
- 🤝 **Shared data** (post tags/mentions) → Anyone with post access can decrypt
- 🗝️ **Key distribution** → Context keys distributed to authorized users
- 🔄 **Consistency** → Same encryption key within each context
...
│        ▼                                                                    │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
│   │  CLOAK ENCRYPTION (Layer 1 - At-Rest)                               │  │
│   │  • Automatic via Encrypted.Binary, Encrypted.Map, etc.              │  │
│   │  • Uses CLOAK_KEY (server environment variable)                     │  │
│   │  • WHERE: Always on server (Fly.io)                                 │  │
│   └─────────────────────────────────────────────────────────────────────┘  │
│        │                                                                    │
│        ▼                                                                    │
│   Double-encrypted blob → Postgres (Fly.io)                                 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
# Reading data (user views a post)
...
### Local SQLite Usage (Cache Only)

For native apps, SQLite is used ONLY for:

1. **Offline Cache** - Store encrypted blobs for offline viewing
2. **Sync Queue** - Queue pending changes when offline
3. **Local Settings** - Non-sensitive app preferences
...
Mosslet supports encryption key rotation without downtime. The system uses Cloak for AES-256-GCM encryption at rest, and supports maintaining multiple cipher configurations for seamless key transitions.

### Environment Variables

- `CLOAK_KEY` - Base encryption key (base64 encoded, 32 bytes)
- `CLOAK_KEY_TAG` - Tag for CLOAK_KEY (default: "AES.GCM.V1")
- `CLOAK_KEY_NEW` - New key for rotation (becomes default when set)
- `CLOAK_KEY_NEW_TAG` - Tag for new key (default: "AES.GCM.V2")
- `CLOAK_KEY_RETIRED` - Additional retired keys (format: "tag1:base64key1,tag2:base64key2")
...
3. **The system automatically**:
   - Uses `CLOAK_KEY_NEW` for all new encryptions
...
4. **Start the rotation job**:
   ...
   |> Mosslet.Security.KeyRotationOrchestratorJob.new()
   |> Oban.insert()
   ```
...
6. **After migration completes**, swap the keys:
   ```bash
   # Move new key to base position
   fly secrets set CLOAK_KEY=<new_key_value> CLOAK_KEY_TAG=AES.GCM.V2
   fly secrets unset CLOAK_KEY_NEW CLOAK_KEY_NEW_TAG
   ```

### Why This Pattern?

This approach ensures you **never lose access to your current key** during rotation:

- ✅ `CLOAK_KEY` always remains set and accessible
- ✅ `CLOAK_KEY_NEW` is additive, not replacing
- ✅ System can decrypt old data while encrypting new data with new key
- ✅ No need to manually track/save the old key value
...
The following schemas have encrypted fields and are included in key rotation:

- Account schemas: User, Connection, UserBlock, UserConnection, UserToken, UserTOTP
- Billing schemas: Customer, PaymentIntent
- Content schemas: Conversation, Group, GroupBlock, GroupMessage, UserGroup
- Memory schemas: Memory, Remark, UserMemory
- Message schemas: Message
- Org schemas: Invitation, Org
- Security schemas: IpBan
- Timeline schemas: Bookmark, BookmarkCategory, ContentWarningCategory, Post, PostHide, PostReport, Reply, UserPost, UserPostReport
