const EntryColumnFlow = {
  ...
  measureAndCreatePages() {
    ...
    const existingPages = wrapper.querySelectorAll(
      '.book-column-page[data-entry-id="' + entryId + '"]'
    );
    ...
    pages.forEach((page, idx) => {
      ...
      clickableDiv.addEventListener("click", () => {
        ...
        const currentPage =
          scrollContainer?.__bookScrollReader?.getCurrentContentPage() || 0;
      ...
      bodyInner.className =
        "h-full prose prose-slate dark:prose-invert max-w-none text-base sm:text-lg leading-relaxed";
  ...
  splitHtmlToFit(html, width, height) {
    ...
    measureDiv.className =
      "prose prose-slate dark:prose-invert max-w-none text-base sm:text-lg leading-relaxed";
    document.body.appendChild(measureDiv);
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (measureDiv.scrollHeight > height) {
        ...
        if (remainingHeight > 20) {
          splitNodeResult = this.splitNodeAtWordBoundary(
            nodes[i],
            measureDiv,
            width,
            remainingHeight
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (measureDiv.scrollHeight > height) {
          ...
          usedChildren.forEach((c) =>
            baseWrapper.appendChild(c.cloneNode(true))
          );
          ...
          if (remainingHeight > 10) {
            splitChildResult = this.splitNodeAtWordBoundary(
              children[i],
              measureDiv,
              width,
              remainingHeight
      ...
      if (splitChildIndex >= 0) {
        if (splitChildResult && splitChildResult.usedHtml.trim().length > 0) {
          ...
          usedChildren.forEach((c) =>
            usedWrapper.appendChild(c.cloneNode(true))
          );
          const usedFragment = document.createElement("div");
          usedFragment.innerHTML = splitChildResult.usedHtml;
          Array.from(usedFragment.childNodes).forEach((c) =>
            usedWrapper.appendChild(c)
          );
          ...
          Array.from(leftoverFragment.childNodes).forEach((c) =>
            leftoverWrapper.appendChild(c)
          );
          children
            .slice(splitChildIndex + 1)
            .forEach((c) => leftoverWrapper.appendChild(c.cloneNode(true)));

          return {
            usedHtml: usedWrapper.outerHTML,
            leftoverHtml: leftoverWrapper.innerHTML.trim()
              ? leftoverWrapper.outerHTML
              : "",
          };
        }
