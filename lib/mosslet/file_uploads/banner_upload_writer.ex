defmodule Mosslet.FileUploads.BannerUploadWriter do
  @moduledoc """
  A LiveView.UploadWriter for banner uploads that processes images and keeps
  ...
  - `{:banner_upload_progress, entry_ref, :receiving, percent}` - Chunk reception
  - `{:banner_upload_progress, entry_ref, :processing, percent}` - Image processing
  - `{:banner_upload_ready, entry_ref, %{temp_path: path, preview_data_url: url}}` - Ready
  - `{:banner_upload_error, entry_ref, reason}` - Error occurred
  """
    ...
    case IO.binwrite(state.file_handle, data) do
      :ok ->
        if state.lv_pid && state.expected_size && state.expected_size > 0 do
          ...
          send(state.lv_pid, {:banner_upload_progress, state.entry_ref, :receiving, percent})
        end
  ...
  def close(state, :done) do
    ...
    result =
      with {:ok, binary} <- File.read(state.temp_path),
           result <- process_banner(binary, state) do
        result
      end
    ...
    case result do
      {:ok, processed_path, preview_data_url} ->
        if state.lv_pid do
          send(
            state.lv_pid,
            {:banner_upload_ready, state.entry_ref,
             %{
      ...
      {:error, reason} ->
        if state.lv_pid do
          send(state.lv_pid, {:banner_upload_error, state.entry_ref, reason})
        end
  ...
  defp process_banner(binary, state) do
    ...
    case result do
      {:ok, webp_binary} ->
        ...
        preview = generate_banner_preview(webp_binary)
  ...
  defp notify_progress(state, stage, value) do
    if state.lv_pid do
      send(state.lv_pid, {:banner_upload_progress, state.entry_ref, stage, value})
    end
