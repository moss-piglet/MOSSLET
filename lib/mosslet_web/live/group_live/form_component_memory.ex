defmodule MossletWeb.GroupLive.FormComponentMemory do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <div class="pb-4">
        <.p :if={@action == :new}>Use this form to create a new memory.</.p>
      ...
      <.form for={@form} id="memory-form" phx-target={@myself} phx-change="validate" phx-submit="save">
        ...
        <div
          :if={
            @selector == "connections" && @action in [:new, :edit] && has_any_user_connections?(@user)
          }
        ...
        <.field :if={@action == :new} field={@form[:blurb]} type="textarea" label="Blurb" />
        <.field
      ...
      case Memories.create_memory(memory_params, user: user, key: key, temp_key: d_conn_key) do
        {:ok, memory} ->
          ...
          if memory.group_id do
            ...
            # rebuild the memory_list
            memories = Memories.list_memories(socket.assigns.user, options)
            loading_list = Enum.with_index(memories, fn element, index -> {index, element} end)
