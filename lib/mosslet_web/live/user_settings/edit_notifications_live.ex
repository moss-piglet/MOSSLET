defmodule MossletWeb.EditNotificationsLive do
    ...
    >
      <DesignSystem.liquid_container max_width="lg" class="py-16">
        ...
        <div class="space-y-8 max-w-3xl">
          ...
          <%!-- Connections-around Card (EPIC #377, task #381) --%>
          <DesignSystem.liquid_card class="bg-gradient-to-br from-teal-50/50 to-emerald-50/30 dark:from-teal-900/20 dark:to-emerald-900/10">
            <:title>
              <div class="flex items-center gap-3">
                <div class="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-lg overflow-hidden bg-gradient-to-br from-teal-100 via-emerald-50 to-teal-100 dark:from-teal-900/30 dark:via-emerald-900/25 dark:to-teal-900/30">
                ...
                <span class={[
                  "inline-flex px-2.5 py-0.5 text-xs rounded-lg font-medium",
                  if(@current_scope.user.show_connections_presence,
                    do:
                      "bg-gradient-to-r from-emerald-100 to-teal-200 text-emerald-800 dark:from-emerald-800 dark:to-teal-700 dark:text-emerald-200 border border-emerald-300 dark:border-emerald-600",
                    else:
                      "bg-gradient-to-r from-slate-100 to-slate-200 text-slate-800 dark:from-slate-700 dark:to-slate-600 dark:text-slate-200 border border-slate-300 dark:border-slate-600"
                  ...
                  {if @current_scope.user.show_connections_presence, do: "On", else: "Off"}
                </span>
              </div>
            </:title>
            ...
            <div class="space-y-6">
              <p class="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
                When turned on, your Home shows a soft, wordless hint when a few of your
                connections happen to be in the app at the same time as you — never a number,
                never a name, just a gentle sense that you're not alone. It's the quiet
                warmth of a shared room, without the pressure to react. Turn it off any time.
              </p>
              ...
              <div class="bg-teal-50 dark:bg-teal-900/20 rounded-lg p-4 border border-teal-200 dark:border-teal-700">
                <div class="flex items-start gap-2 text-sm text-teal-700 dark:text-teal-300">
                  ...
                  <span>
                    You'll only ever sense connections who've chosen to share their presence
                    with you. There are no counts, no names, and no notifications — nothing
                    leaves your device on our side.
                  </span>
                </div>
              </div>

              <.form
                id="update_connections_presence_form"
                for={@connections_presence_form}
                phx-change="toggle_connections_presence"
                class="space-y-6"
              >
                <DesignSystem.liquid_checkbox
                  field={@connections_presence_form[:show_connections_presence]}
                  label="Let me sense when my people are around"
                  help={
                    if @current_scope.user.show_connections_presence,
                      do: "Turn off to stop seeing the who's-around hint on your Home.",
                      else:
                        "Turn on to see a calm, content-free hint on your Home when connections are present."
                  }
                />
              </.form>
            </div>
  ...
  def handle_event("toggle_connections_presence", %{"user" => user_params}, socket) do
    enabled = user_params["show_connections_presence"] == "true"

    case Accounts.update_connections_presence_enabled(socket.assigns.current_scope.user, enabled) do
      {:ok, current_user} ->
        socket =
          socket
          |> put_flash(
            :success,
            gettext("Your who's-around preference has been updated.")
          )
          |> assign(
            current_scope: Scope.for_user(current_user, key: socket.assigns.current_scope.key)
          )
